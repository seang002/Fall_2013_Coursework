...
sub solver {
    my ($coefficient0, $coefficient1, $delta) = @_;
    my $root1 = (-$coefficient1+sqrt($delta))/(2*$coefficient0);
    my $root2 = (-$coefficient1-sqrt($delta))/(2*$coefficient0);
    return($root1,$root2);
}
...
if ($delta < 0) {
    ...
    my ($root1, $root2) = &solver(@coefficients, $delta);
